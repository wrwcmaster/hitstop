...
**Online co-op, no server:** pick **CO-OP** on the title screen. The host sends an invite code to a friend over any chat; the friend replies with their code; paste it back and the two browsers connect *directly* (WebRTC peer-to-peer). The guest's knight fights in the host's world — kills, XP, and drops are shared. Copy-paste signaling means zero infrastructure, though very strict NATs may fail to connect (there's no relay server, by design).
