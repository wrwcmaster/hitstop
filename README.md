...
**Classes:** pick your road in the SKILL TREE screen (Esc → SKILL TREE, then up to the class tabs) — **KNIGHT** (harder hits, more hearts, dash strike), **MAGE** (deep mana, learned NOVA, frost tricks), or **TIDECALLER** (faster on land, double jump, gale dash, and the deep-water tier). Each class has its own small skill tree; switching is free and keeps every class's unlocks parked for your return, with skill points as one shared pool — so experiment, don't grind.

**Puzzles:** the **Town Vault** (a heavy door on Haven's east edge) is the old kings' trap-gauntlet — **spike pits** that cost a heart and launch you clear, **moving platforms** that carry you over them, a **lever** that latches a barrier open, and a **pressure plate** whose barrier only *lingers* a few seconds after you step off: press it, then sprint. Three treasure chests wait at the end. All of it is data — platforms, levers, plates, and barriers are placeables wired by named switch flags, so the level editor can build new puzzles with zero code.
