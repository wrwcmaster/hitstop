...
// Extract rows 0 to 30 of the user's redesigned unequipped idle frame 0
const newHeadRows = knight.anims.idle.frames[0].slice(0, 31);
...
for (const [animName, animData] of Object.entries(knight.anims) as [string, any][]) {
  ...
  animData.frames.forEach((frame: string[], fIdx: number) => {
    ...
    if (shift === 0) {
      // Replace rows 0 to 30
      for (let y = 0; y <= 30; y++) {
        frame[y] = newHeadRows[y];
      }
    } else if (shift === 1) {
      // Set row 0 to transparent
      frame[0] = '.'.repeat(35);
      // Replace rows 1 to 31 with head rows 0 to 30
      for (let y = 0; y <= 30; y++) {
        frame[y + 1] = newHeadRows[y];
      }
    }
  });
}
...
console.log('Successfully updated all other frames in knight.json to match the new unequipped idle frame!');
