...
export interface SongTrack {
  /** Oscillator type, or 'noise' for percussion hits. */
...
export class Music {
  ...
  private scheduleStep(def: SongDef, step: number, when: number, stepDur: number): void {
    for (const track of def.tracks) {
      ...
      if (track.wave === 'noise') this.noiseHit(when, dur, track.volume);
      else this.note(track.wave, noteHz(cell), when, dur, track.volume);
    }
  }
