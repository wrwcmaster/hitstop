...
- **Gear visuals**: armor-like equipment uses a layer registry (`content/gear-visuals.ts`) keyed by item id; each entry declares its equipment slot, layer order, icon, and optional foreground grip colors. This lets two helmets in one slot keep distinct art. Weapons use `weapon-visuals.ts`, a separate registry that owns held art, hand usage, and attack trails while sharing the same optional frame-aligned sprite-sheet approach.
