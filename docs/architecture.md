...
| `ui/` | `drawPanel`/`Menu`/`drawMeter` widgets, `DialogueScene` (typewriter + choices), `Minimap` (baked tiles + live markers) | Conversations are data in a registry; menus are the same widget everywhere. `drawMeter` draws a row of segment icons (hearts, mana pips, any counted resource) where the segment being spent shows a PARTIAL fill — clipped to the value's fractional remainder — instead of only ever losing a whole segment. That's what lets two different attacks read as two different amounts of damage without new art per fraction; see `math/util.ts`'s `formatAmount` for the matching numeric readout. |
