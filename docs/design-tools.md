...
Source: `tools/sprite-editor.html` + `tools/src/sprite-editor.ts`. The document
invariants, transaction boundary, and extension rules are documented in
[sprite-editor-architecture.md](sprite-editor-architecture.md).
...
- **select / clipboard** - drag a pixel rectangle, or use **Magic (W)** to select a contiguous color region with adjustable tolerance. Magic can replace, add, subtract, or intersect; Shift adds, Alt subtracts, and Shift+Alt intersects without changing the configured mode. Mask-shaped selections remain masks through cut/copy/paste, move, resize, rotation, undo, and the agent bridge. Paste uses the selection's top-left as its destination and clips safely at the frame edge. Escape or right-click clears the selection.
- **fill / color drift** - ordinary Fill (G) replaces an exact connected palette color. Its contextual **match selected color drift** mode treats the clicked pixel as the source and the active palette swatch as the target, then adds that RGB difference to every opaque pixel in the selection. This maps the source exactly while moving highlights and shadows together, which is useful for harmonizing drifting scarf, cloth, or armor colors across generated frames.
...
- **persistent preview** — the live sprite/composite canvas and a compact context label sit above the right workspace and remain visible while editing any tab. **Animate** owns animation and frame controls, **Combat** owns attack timing and regions, **Layers** owns the shared layer stack, and **Rig** owns size, hitbox, anchors, and attachment slots. Composite/reference controls share the left **Compose** page. Transform controls are contextual tool options, so they appear beside the canvas only while a selection is in transform mode. JSON exchange opens from **Menu** instead of occupying a permanent inspector tab.
- **professional workspace** — the header names the active document and reports unsaved workspace sets; stable icon groups keep tools from shifting when state changes; and the bottom status bar continuously reports the active tool, pointer coordinates, animation/frame, layer, canvas size, and zoom. Left/right panel toggles preserve canvas position. `Tab` hides or restores both panels, while **Menu → reset workspace** restores the default panels, pages, and fitted canvases.
- **Menu** — Undo, Redo, Cut, Copy, Paste, Select All, Deselect, workspace commands, layer tags, sprite data, and the shortcut guide stay available without permanently occupying the header or inspector. Their Ctrl/Cmd shortcuts remain the fastest path.
- **animations** — a button per animation in **Animate**; click to edit it. A linked animation is shown as `name → source` and borrows the source timeline. **unlink** materializes it as an independent animation by cloning timing, every layer track, and frame-aligned anchors in one undoable operation. **+ anim** / **rename** / **del**, and an **fps** for the selected one.
...
- **top toolbar** — keeps compact icon buttons for the immediate canvas tools, selection status, zoom out, an editable percentage, zoom in, **fit**, and panel visibility without mixing in operation-specific controls. Brush and blur size live in the contextual **Tool** page. Tooltips, pressed-state accessibility, and the live tool description retain each tool name, shortcut, and interaction.
- **navigation shortcuts** — hold `Space` and drag to pan without changing tools or dismissing a selection; `Ctrl/Cmd +` and `Ctrl/Cmd -` zoom around the visible center; `Ctrl/Cmd 0` fits; `Ctrl/Cmd A` selects the canvas; `Ctrl/Cmd D` deselects; and `?` opens the complete in-editor shortcut guide. Inspector tabs support Left/Right/Home/End when focused.
...
- `GET /__sprite-editor/selection` returns the active animation, frame, bounds, exact text-grid rows, and an optional `1`/`.` mask for non-rectangular selections. Selection is transient collaboration metadata: pointing at art never dirties the sprite or advances its document revision.
